...
def Response(input1,input2):
    results = (input1 +  input2) / 2




while True:
 pubnub.publish().channel("photo").message(input("->")).async(my_publish_callback)


pubnub.add_listener(MySubscribeCallback())
pubnub.subscribe().channels('results').execute()
